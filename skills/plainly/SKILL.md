...
The cold read is off. It sent each draft commit message and explanation to a subagent without session context, then rewrote the draft from that subagent's answer. `references/cold-read.md` keeps the procedure. Leave that file unloaded until John turns the cold read back on.
