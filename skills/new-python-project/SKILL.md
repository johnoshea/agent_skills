...
5. Write `tests/test_<module>.py` from `templates/test_smoke.py`, replacing `MODULE` with the directory name uv created under `src/`. pytest fails with no tests, so this test is what lets the first commit pass.
6. `prek install`.
...
- **ruff.** Line length 88, `E501` off because `ruff format` owns line length, mccabe cyclomatic complexity capped at 10 through `C90`. Tests may use `assert`, nested `with`, and temporary paths.
